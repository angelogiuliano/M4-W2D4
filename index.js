    ...
    .then((data) => {
      ...
      for (let i = 0; i < cartButtons.length; i++) {
        cartButtons[i].addEventListener("click", function () {
          if (!listaCarrello.includes(this.parentElement.attributes.id.value)) {
            listaCarrello.push(this.parentElement.attributes.id.value);
            console.log("aggiunto");
          } else {
            const index = listaCarrello.indexOf(this.parentElement.attributes.id.value);
            if (index > -1) {
